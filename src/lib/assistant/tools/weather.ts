...
const geocodeCache = new Map<string, { lat: number; lon: number; expires: number }>();
...
export const weatherTool: Tool = {
  ...
  execute: async (args) => {
    ...
    if (locationName !== DEFAULT_LOCATION.name) {
      const geocodeKey = locationName.toLowerCase().trim();
      const now = Date.now();
      const cachedGeo = geocodeCache.get(geocodeKey);
    ...
    const weatherKey = getWeatherCacheKey(latitude, longitude);
    const now = Date.now();
    ...
    try {
      ...
      weatherCache.set(weatherKey, { output, expires: now + CACHE_TTL_WEATHER });
