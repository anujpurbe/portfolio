...
function scroll(target: string, label: string): AssistantAction {
  return { label, type: "scroll", target };
}

function projectResult(p: Project): AssistantResult {
  return {
    type: "project",
    id: p.slug,
    title: p.title,
...
function certificateResult(c: (typeof certifications)[number]): AssistantResult {
  return {
    type: "certificate",
    id: c.title,
    title: c.title,
...
function allProjects(): AssistantResponse {
  return {
...
function allCertificates(): AssistantResponse {
  return {
...
function allSkills(): AssistantResponse {
  return {
...
function resumeResponse(): AssistantResponse {
  return {
    answer: "Here's Anuj's resume — it opens as a PDF in a new tab.",
    actions: [{ label: "View resume", type: "resume", href: site.resume }],
  };
}

function githubResponse(): AssistantResponse {
  return {
    answer:
...
function instagramResponse(): AssistantResponse {
  return {
    answer: "You can find Anuj on Instagram here.",
    actions: [
      { label: "Instagram", type: "external", href: site.socials.instagram.href },
      scroll("contact", "Contact"),
    ],
  };
}

function linkedinResponse(): AssistantResponse {
  return {
    answer:
      "Anuj's LinkedIn is linkedin.com/in/anuj-purbe — connect there for professional updates.",
    actions: [
      { label: "LinkedIn", type: "external", href: site.linkedin },
      scroll("contact", "Contact"),
    ],
  };
}

function socialsResponse(): AssistantResponse {
  const socials = [
    { label: "GitHub", href: site.github },
    { label: "LinkedIn", href: site.linkedin },
    { label: "Instagram", href: site.socials.instagram.href },
  ];
  return {
    answer: "Here are Anuj's social links — GitHub, LinkedIn, and Instagram.",
    actions: [
...
function educationResponse(): AssistantResponse {
...
function learningResponse(): AssistantResponse {
...
function internshipResponse(): AssistantResponse {
  return {
    answer:
...
function statsResponse(): AssistantResponse {
  return {
...
function achievementsResponse(): AssistantResponse {
  return {
    answer: achievements
      .map(
        (a) =>
...
// Exported so the engine can detect "unknown" portfolio answers and decide
// whether an AI summarization is worth the quota.
...
function isFallback(resp: AssistantResponse): boolean {
  return resp.answer === FALLBACK_TEXT;
}
...
function identityResponse(): AssistantResponse {
  return {
    answer:
...
function capabilitiesResponse(): AssistantResponse {
  return {
    answer:
...
function statusResponse(): AssistantResponse {
  return {
...
function thanksResponse(): AssistantResponse {
  return {
    answer:
...
function goodbyeResponse(): AssistantResponse {
  return {
    answer:
...
function exploreResponse(): AssistantResponse {
  return {
    answer:
...
function interestingResponse(): AssistantResponse {
  const facts = [
...
function anaphoraResponse(q: string, prevUser: string): AssistantResponse | null {
  const referencesIt =
    /(^|\s)(it|that|this|that one|this one|the project|that project)(\s|$)/.test(q) ||
...
const localCache = new Map<string, AssistantResponse>();

function cachedAnswer(
  q: string,
  prevUser: string,
  compute: () => AssistantResponse,
): AssistantResponse {
...
function computeAnswer(q: string, raw: string, prevUser: string): AssistantResponse {
  if (!q) {
    return {
      answer:
...
export function isFallbackResponse(resp: AssistantResponse): boolean {
  return isFallback(resp);
}